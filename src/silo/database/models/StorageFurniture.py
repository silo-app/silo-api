...
class StorageFurniture(Base, TimestampMixin):
    ...
    storage_type_id: Mapped[int] = mapped_column(ForeignKey("storage_type.id"), nullable=False)

    __table_args__ = (
        UniqueConstraint("name", "room_id", "storage_type_id", name="unique_storage_furniture"),
    )